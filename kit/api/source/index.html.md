...
# Introduction
OST KYC is the first plug-and-play kit/AML management solution for token sales to process
...
As part of the complete solution one of the product offerings is the robust set of OST KYC APIs. They allow access to several functionality used for kit/AML verification. Here are the APIs that we deliver through OST KYC.
...
A POST to `https://kyc.ost.com/api/v2/users` creates a new `user` object for the user in OST KYC database. Only user's signup information is sent via this endpoint. For sending user's KYC details a POST request to a different endpoint `/users-kit/{{user_id}}` has to be sent.
...
def submit_kyc(user_id, custom_params={})
  endpoint = "/api/v2/users-kit/#{user_id}"
...
```
For POST calls to `/users-kit/{{user_id}}` the data.result\_type is the string "user_kyc" and the key data.user\_kyc is an **user-kyc** object if a valid user identifier was provided. A `user-kyc` object provides properties and status related information of the kyc details that a user had last submitted.
...
<u>**Returns**</u><br>
For api calls to `/users-kit/{{user_id}}` the data.result\_type is the string "user_kyc" and the key data.user\_kyc is an user-kyc object if a valid identifier was provided. When the requesting ID of a user is not found a 404, resource could not be located error will be returned.
...
def get_presigned_url_put(custom_params)
  endpoint = "/api/v2/users-kit/pre-signed-urls/for-put"
  make_get_request(endpoint, custom_params)
end
...
A GET to `https://kyc.ost.com/api/v2/users-kit/pre-signed-urls/for-put` will generate the pre-signed URL for uploading the documents which we return in the response. We also show example code for consuming the response this endpoint sends.
...
<u>**Returns**</u><br>
For api calls to `/users-kit/pre-signed-urls/for-put` the data.result\_type is the string "file\_upload\_put" and the key data.file\_upload_put is a `file_upload_put` object. The pre-signed URLs will be sent against the unique key. The pre-signed URLs are generated with an expiration time of 15 minutes after which they can not used anymore. 

For instance, the value in the key `data.file_upload_put.document_id.fields.key` in the response will be used to set the value of input parameter `document_id_file_path` while adding or updating kyc details for a user using the endpoint `/users-kit/{{user_id}}`
...
def get_presigned_url_post(custom_params)
  endpoint = "/api/v2/users-kit/pre-signed-urls/for-post"
  make_get_request(endpoint, custom_params)
end
...
A GET to `https://kyc.ost.com/api/v2/users-kit/pre-signed-urls/for-post` will generate the pre-signed URL for uploading the documents which we return in the response. It allows to upload documents to S3 directly from browser using an HTML form. 
...
Also the value in the key `data.file_upload_post.document_id.fields.key` in the response will be used to set the value of input parameter `document_id_file_path` while adding or updating kyc details for a user using the endpoint `/users-kit/{{user_id}}`
